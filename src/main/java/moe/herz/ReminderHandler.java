...
public class ReminderHandler {
    ...
    public void init() {
        ...
        try (Statement stmt = dbConnection.createStatement()) {
            ResultSet rs = stmt.executeQuery("SELECT id, remind_at FROM LoreleyReminder");
            while (rs.next()) {
    ...
    public void addReminder(String sender, String message, Instant remindAt, String channel) {
        try (PreparedStatement pstmt = dbConnection.prepareStatement("INSERT INTO LoreleyReminder (sender, message, remind_at, channel) VALUES (?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS)) {
    ...
    public SimpleEntry<String, String> fetchReminder(long reminderId) {
        ...
        String channel = null;
        try (PreparedStatement pstmt = dbConnection.prepareStatement("SELECT sender, message, channel FROM LoreleyReminder WHERE id = ?")) {
    ...
    public void removeReminder(long reminderId) {
        ...
        if (reminder != null) {
            ...
            try (PreparedStatement pstmt = dbConnection.prepareStatement("DELETE FROM LoreleyReminder WHERE id = ?")) {
                pstmt.setLong(1, reminderId);
                pstmt.executeUpdate();
            } catch (SQLException e) {
                e.printStackTrace();
